...
async def Login_to(page):
    try:
        acct_balance = await page.waitForSelector('span[id="j_balance"]', timeout=7000)
        spt_acct_balance = (await (await acct_balance.getProperty('textContent')).jsonValue()).strip()
        if spt_acct_balance:
    ...
    now_balance = await page.waitForSelector('span[id="j_balance"]', timeout=7000)
    spt_acct_balance = (await (await now_balance.getProperty('textContent')).jsonValue()).strip()
    if spt_acct_balance:
        print(f"\n>>> ALREADY LOGGED IN! ACCOUNT BALANCE: {spt_acct_balance} <<<\n")